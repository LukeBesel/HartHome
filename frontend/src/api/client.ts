...
export const api = {
  ...
  forgotPassword: (email: string) => post<{ ok: boolean }>('/auth/forgot', { email }),
  resetPassword: (token: string, password: string) => post<{ ok: boolean }>('/auth/reset', { token, password }),
  ...
  upload: (name: string, data: string) => post<{ url: string; size: number; mime: string }>('/upload', { name, data }),
  icalFeed: () => get<{ path: string }>('/ical/feed'),
  regenerateIcalFeed: () => post<{ path: string }>('/ical/feed/regenerate'),
